...
const UserModel = require("../models/diary.model");
const APIError = require("../helpers/api-error");
const logger = require("../helpers/logger");

const createUsers = async ({date, work, starttime, endtime, reviews}) => {
  let session = null;
  try {
...
const updateUser = async ({ id, date, work, starttime, endtime, reviews }) => {
  try {
    const user = await UserModel.findById(id);

    user.overwrite({ date, work, starttime, endtime, reviews });
    await user.save();
